 ...
 */
public class KeyVaultSecretsLogPolicy implements HttpPipelinePolicy {
  private static final ObjectMapper PRETTY_PRINTER = new ObjectMapper()
        .enable(SerializationFeature.INDENT_OUTPUT);
  private static final int MAX_BODY_LOG_SIZE = 1024 * 16;
  private static final String REDACTED_PLACEHOLDER = "REDACTED";
  private static final String RIGHT_ARROW_END = "--> END ";
  private static final String LEFT_ARROM_END = "<-- END HTTP";
  private final HttpLogDetailLevel httpLogDetailLevel;
  private final Set<String> allowedHeaderNames;
  private final Set<String> allowedQueryParameterNames;
  private final boolean prettyPrintBody;
  private Pattern secretsPattern;

  /**
   * Creates an KVSecretsLoggingPolicy with the given log configurations.
   *
   * @param httpLogOptions The HTTP logging configuration options.
   */
  public KeyVaultSecretsLogPolicy(HttpLogOptions httpLogOptions) {
    this.secretsPattern = Pattern.compile("(\"value\":\")(.*?)(\")");
  ...
  public Mono<HttpResponse> process(HttpPipelineCallContext context, HttpPipelineNextPolicy next) {
    ...
    final ClientLogger logger = new ClientLogger((String) context
        .getData("caller-method")
        .orElse(""));
      ...
      request.setBody(
          request.getBody()
              ...
              .doFinally(ignored -> {
                ...
                body = secretsPattern.matcher(body).replaceAll("$1****$3");
                requestLogMessage.append(contentLength)
                    .append("-byte body:")
                    .append(System.lineSeparator())
                    .append(prettyPrintIfNeeded(logger, contentType, body))
                    .append(System.lineSeparator())
                    .append(RIGHT_ARROW_END)
                    .append(request.getHttpMethod())
                    .append(System.lineSeparator());
   ...
   */
  private Mono<HttpResponse> logResponse(final ClientLogger logger, 
        final HttpResponse response, long startNs) {
    if (!logger.canLogAtLevel(LogLevel.INFORMATIONAL)) {
    ...
    if (!httpLogDetailLevel.shouldLogBody()) {
      responseLogMessage.append(LEFT_ARROM_END);
   ...
   */
  private void addHeadersToLogMessage(ClientLogger logger, 
        HttpHeaders headers, StringBuilder sb) {
    // Either headers shouldn't be logged or the log level isn't set to VERBOSE, don't add headers.
   ...
   *
   * The body is pretty printed if the Content-Type is JSON and 
   * the policy is configured to pretty print JSON.
   * 
   ...
   *
   * The request or response body is logged if the Content-Type is not "application/octet-stream" 
   * and the body isn't empty and is less than 16KB in size.</p>
   *
